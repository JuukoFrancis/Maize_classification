    ...
    try:
        with requests.get(url, headers=get_hf_headers(), stream=True, timeout=600) as r:
            if r.status_code == 401:
                st.error(
                    f"❌ **{name}**: Authentication failed (HTTP 401). "
                    f"Make sure `HF_TOKEN` is set in Streamlit secrets and your token has read access."
                )
                return False
            if r.status_code == 403:
                st.error(
                    f"❌ **{name}**: Access forbidden (HTTP 403). "
                    f"Your token may not have permission to access this repo."
                )
                return False
            if r.status_code == 404:
                probe = probe_url(url)
                st.error(
                    f"❌ **{name}**: File not found (HTTP 404).\n\n"
                    f"**URL tried:** `{url}`\n\n"
                    f"**Probe result:** `{probe}`\n\n"
                    f"**Checklist:**\n"
                    f"- Is the repo type correct? Currently set to `HF_REPO_TYPE = '{HF_REPO_TYPE}'`. "
                    f"Try changing it to `'model'` if your repo is a model repo (or vice versa).\n"
                    f"- Does the filename `{MODEL_FILES[name]}` exactly match what's on Hugging Face "
                    f"(check capitalisation and extension)?\n"
                    f"- Is the repo public, or is `HF_TOKEN` set correctly?"
                )
                return False
            r.raise_for_status()
            with open(path, "wb") as f:
                for chunk in r.iter_content(chunk_size=1024 * 64):
                    if chunk:
                        f.write(chunk)
        # Reject tiny files (HTML error pages served instead of model binary)
        if os.path.getsize(path) < 1_000_000:
            with open(path, "rb") as f:
                snippet = f.read(200).decode("utf-8", errors="replace")
            os.remove(path)
            st.error(
                f"❌ **{name}**: Downloaded file is too small — likely an HTML error page.\n\n"
