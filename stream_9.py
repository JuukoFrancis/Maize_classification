...
================================================================
  ...
  Model Storage: Hugging Face (JuukoFrancis/Maize_models)
================================================================
  Run:
      pip install streamlit tensorflow pillow scikit-learn matplotlib seaborn requests
      streamlit run app.py

  Streamlit Secrets (add in dashboard → App settings → Secrets):
...
#  HUGGING FACE URLS
#
#  FIX: Two possible base URLs depending on whether you uploaded
#  to a DATASET repo or a MODEL repo on Hugging Face.
#
#  Dataset repo URL pattern:
#    https://huggingface.co/datasets/<user>/<repo>/resolve/main/<file>
#  Model repo URL pattern:
#    https://huggingface.co/<user>/<repo>/resolve/main/<file>
#
#  Check which one your repo is at:
#    https://huggingface.co/datasets/JuukoFrancis/Maize_models  ← dataset
#    https://huggingface.co/JuukoFrancis/Maize_models           ← model
#
#  Set HF_REPO_TYPE below to "dataset" or "model" accordingly.
# ══════════════════════════════════════════════════════════════
HF_REPO_TYPE = "dataset"   # ← change to "model" if it's a model repo

HF_USER = "JuukoFrancis"
HF_REPO = "Maize_models"

if HF_REPO_TYPE == "dataset":
    HF_BUCKET_BASE = f"https://huggingface.co/datasets/{HF_USER}/{HF_REPO}/resolve/main"
else:
    HF_BUCKET_BASE = f"https://huggingface.co/{HF_USER}/{HF_REPO}/resolve/main"

# ── Exact filenames as they appear on Hugging Face (case-sensitive!) ──
MODEL_FILES = {
...
# ══════════════════════════════════════════════════════════════
#  HF TOKEN — reads from env var OR Streamlit secrets safely
#
#  To add your token in Streamlit Cloud:
#    App settings → Secrets → paste:
#      HF_TOKEN = "hf_xxxxxxxxxxxxxxxxxxxx"
# ══════════════════════════════════════════════════════════════
def get_hf_token() -> str | None:
    # 1. Environment variable (works locally and on many cloud platforms)
...
# ══════════════════════════════════════════════════════════════
#  DEBUG HELPER — shown only when a download fails
#  Probes the URL and reports the HTTP status so you can
#  distinguish 404 (wrong URL/filename) from 401 (auth issue).
# ══════════════════════════════════════════════════════════════
def probe_url(url: str) -> str:
    try:
        r = requests.head(url, headers=get_hf_headers(), timeout=15, allow_redirects=True)
...
def download_model(name: str, path: str) -> bool:
    ...
    # HF Git LFS files need ?download=true to get the binary, not the pointer file.
    # We also try both dataset and model repo URL patterns as fallbacks.
    candidate_urls = [
        f"{HF_BUCKET_BASE}/{fname}?download=true",                                         # LFS direct (dataset repo)
        f"{HF_BUCKET_BASE}/{fname}",                                                        # plain resolve (dataset repo)
        f"https://huggingface.co/{HF_USER}/{HF_REPO}/resolve/main/{fname}?download=true",  # LFS direct (model repo)
        f"https://huggingface.co/{HF_USER}/{HF_REPO}/resolve/main/{fname}",                # plain resolve (model repo)
    ]
    ...
    for url in candidate_urls:
        try:
            ...
            # Reject LFS pointer files and HTML error pages (< 1 MB)
            if os.path.getsize(path) < 1_000_000:
                ...
                if "git-lfs" in snippet:
                    st.warning(f"⚠️ **{name}**: Got an LFS pointer instead of the binary. Trying next URL…")
                continue   # try next URL
    ...
    # All URLs failed — report clearly
    probes = "\n".join(f"  - `{u}` → `{probe_url(u)}`" for u in candidate_urls)
    st.error(
        f"❌ **{name}**: Could not download from any URL.\n\n"
        f"**URLs tried:**\n{probes}\n\n"
        f"**Most likely causes:**\n"
        f"1. The repo is private and `HF_TOKEN` is missing/wrong — check Streamlit secrets.\n"
        f"2. The filename `{fname}` doesn't match exactly what's on Hugging Face.\n"
        f"3. The file wasn't uploaded properly (re-upload via `git lfs push`).\n\n"
        f"**Quick check:** Open this URL in your browser while logged in to HF:\n"
        f"`https://huggingface.co/datasets/{HF_USER}/{HF_REPO}/resolve/main/{fname}?download=true`"
    )
    return False


def ensure_models_downloaded():
    """Download missing models from HF. Cached valid files are reused."""
    os.makedirs("models", exist_ok=True)
    status = {}

    # Show which token is in use (masked) to help debug auth issues
    token = get_hf_token()
    if token:
        masked = token[:8] + "…" + token[-4:]
        st.sidebar.caption(f"🔑 HF token detected: `{masked}`")
    else:
        st.sidebar.caption("⚠️ No HF token found — only public repos will work.")
    ...
    for name, path in MODEL_PATHS.items():
        ...
        banner = st.info(f"⬇️ Downloading **{name}** from Hugging Face…", icon="☁️")
        ok = download_model(name, path)
        if ok:
            mb = os.path.getsize(path) / 1_000_000
            banner.success(f"✅ **{name}** ready — {mb:.1f} MB")
        else:
            banner.empty()   # clear the spinner; error was already shown by download_model
        status[name] = ok
...
with st.sidebar:
    ...
    st.markdown("<h3 style='color:#f9fafb;font-family:Playfair Display,serif;'>☁️ Model Files</h3>",
                unsafe_allow_html=True)
    st.markdown(
        f"<small style='color:#9ca3af;'>HF {HF_REPO_TYPE.title()} · {HF_USER}/{HF_REPO}</small>",
        unsafe_allow_html=True)
